...
# -*- coding: utf-8 -*-
"""
Monte Carlo HTML Generator V2.1
================================

Génère des rapports HTML pour les simulations Monte Carlo:
- Page de synthèse: all_strategies_montecarlo.html (AVEC RECALCUL DYNAMIQUE)
...
Auteur: Yann
Date: 2025-12-01
Version: 2.1 - Ajout du dashboard interactif avec recalcul dynamique
"""
...
# NOUVEAU: Importer la configuration du dashboard
try:
    ...
    print("⚠️ Warning: Impossible d'importer la config du dashboard, utilisation des valeurs par défaut")
    STATUS_OK = "OK"
    STATUS_WARNING = "WARNING"
    STATUS_HIGH_RISK = "HIGH_RISK"
    DASHBOARD_DEFAULT_CRITERIA = {'max_ruin': 0.10, 'min_return_dd': 2.0, 'min_prob_positive': 0.80}
    DASHBOARD_PRESETS = {}
    DASHBOARD_COLORS = {}
    SLIDER_RANGES = {}
    DASHBOARD_DISPLAY = {'min_trades_default': 20, 'animation_duration_ms': 500}
    FILE_PATTERNS = {}
...
def generate_summary_html(
    ...
    warning_count = status_counts.get(STATUS_WARNING, 0)
    high_risk_count = status_counts.get(STATUS_HIGH_RISK, 0)
    ...
    for _, row in summary_df.iterrows():
        status_class = {
            ...
            STATUS_HIGH_RISK: 'status-danger'
        }.get(row['status'], '')
    ...
    # NOUVEAU: Préparer les données de configuration pour le template
    presets_json = json.dumps(DASHBOARD_PRESETS) if DASHBOARD_PRESETS else '{}'
    colors_json = json.dumps(DASHBOARD_COLORS) if DASHBOARD_COLORS else '{}'
    slider_ranges_json = json.dumps(SLIDER_RANGES) if SLIDER_RANGES else '{}'
    default_criteria_json = json.dumps(DASHBOARD_DEFAULT_CRITERIA) if DASHBOARD_DEFAULT_CRITERIA else '{}'
    ...
    html_content = HTML_SUMMARY_TEMPLATE.format(
        ...
        # NOUVEAUX placeholders pour la configuration
        presets_json=presets_json,
        colors_json=colors_json,
        slider_ranges_json=slider_ranges_json,
        default_criteria_json=default_criteria_json,
    )
...
def main(run_dir: Optional[Path] = None):
    ...
    """
    print("=" * 80)
    print("GÉNÉRATEUR DE RAPPORTS HTML MONTE CARLO V2.1")
    print("=" * 80)
    print()
